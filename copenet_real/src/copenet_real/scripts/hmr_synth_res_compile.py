...
images0 = np.array([os.path.join("/mnt/d/data/copenet_synthetic",pkl.load(open(f,"rb"))["im0"]) for f in test_ds.db])
...
joints3d = [] 
for i in tqdm(range(pred_rotmat0_test.shape[0])):
...
import cv2
import random
samples = random.sample(range(begin*batch_size,batch_size*(end-begin)),1)  # 原来是 5
