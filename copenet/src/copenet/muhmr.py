...
class muhmr(pl.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        self.model = model_muhmr.getcopenet(os.path.join(self.hparams.copenet_home,"src/copenet/data/smpl_mean_params.npz"))
    ...
    def fwd_pass_and_loss(self,input_batch,is_test=False, is_val=False):
        ...
        if is_test:
            ...
            pred_vertices0,pred_joints_cam0,_,_ = transform_smpl(transf_mat0,
                                                pred_output_cam0.vertices.squeeze(1),
                                                pred_output_cam0.joints.squeeze(1))
            ...
            pred_vertices1,pred_joints_cam1,_,_ = transform_smpl(transf_mat1,
                                                pred_output_cam1.vertices.squeeze(1),
                                                pred_output_cam1.joints.squeeze(1))


            in_cam_t = torch.stack([self.model.init_cam[0,1].expand(batch_size),
                                  self.model.init_cam[0,2].expand(batch_size),
                                  2*self.focal_length[0]/(self.hparams.img_res * self.model.init_cam[0,0].expand(batch_size) +1e-9)],dim=-1)
            in_cam_trans0 = torch.bmm(torch.inverse(intr0),torch.bmm(modif_intr0,in_cam_t.unsqueeze(2)))
            in_cam_trans_z0 = (in_cam_t/((self.focal_length[0]/bb0[:,2])/self.focal_length[0]).unsqueeze(1))[:,2]
            in_cam_trans0 = (in_cam_trans0.squeeze(2)*in_cam_trans_z0.unsqueeze(1)/in_cam_trans0[:,2])
            in_cam_trans1 = torch.bmm(torch.inverse(intr1),torch.bmm(modif_intr1,in_cam_t.unsqueeze(2)))
            in_cam_trans_z1 = (in_cam_t/((self.focal_length[0]/bb1[:,2])/self.focal_length[0]).unsqueeze(1))[:,2]
            in_cam_trans1 = (in_cam_trans1.squeeze(2)*in_cam_trans_z1.unsqueeze(1)/in_cam_trans1[:,2])


            output = {'pred_vertices_cam0': pred_vertices0.detach().cpu(),
                        'pred_vertices_cam1': pred_vertices1.detach().cpu(),
                        'pred_smpltrans0': pred_cam_trans0.detach().cpu(),
                        'pred_smpltrans1': pred_cam_trans1.detach().cpu(),
                        "pred_j3d_cam0": pred_joints_cam0.detach().cpu(),
                        "pred_j3d_cam1": pred_joints_cam1.detach().cpu(),
                        'in_smpltrans0': in_cam_trans0.detach().cpu(),
                        'in_smpltrans1': in_cam_trans1.detach().cpu(),
                        'gt_smpltrans0': input_batch["smpltrans_rel0"].detach().cpu(),
                        'gt_smpltrans1': input_batch["smpltrans_rel1"].detach().cpu(),
                        "pred_angles0": pred_angles0.detach().cpu(),
                        "pred_betas0": pred_betas0.detach().cpu().cpu(),
                        "pred_betas1": pred_betas1.detach().cpu().cpu(),
    ...
    def add_model_specific_args(parent_parser):
        ...
        train.add_argument('--summary_steps', type=int, default=850, help='Summary saving frequency')
        ...
        train.add_argument('--shape_loss_weight', default=100, type=float, help='Weight of per-vertex loss') 
        train.add_argument('--keypoint2d_loss_weight', default=0.05, type=float, help='Weight of 2D keypoint loss')
        ...
        train.add_argument('--pose_loss_weight', default=100, type=float, help='Weight of SMPL pose loss') 
